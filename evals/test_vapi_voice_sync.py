"""vapi_voice_sync.py — shared VAPI voice-PATCH mechanics used by both
scripts/sync_vapi_voice.py and platform_api/voice_sync.py.

Never hits the real VAPI network: vapi_voice_sync.vapi_api is the one seam
both plan_assistant_voice (GET) and apply_assistant_voice (PATCH + verify
GET) go through, monkeypatched here with a fake recorder.
...
once, not reimplemented twice):
  ...
  2. The computed `after` payload preserves every voice key it didn't intend
     to touch (provider, stability, similarityBoost, ...).
  3. apply_assistant_voice no-ops (applied=False, verified=True) when the
     plan wasn't changed — PATCH is never called for an already-in-sync
     assistant.
  4. apply_assistant_voice PATCHes then re-GETs to verify; a mismatch after
     PATCH is reported as verified=False, not silently treated as success.
  5. A VAPI HTTP failure during apply is caught and reported on the result
     (error set, applied=False) rather than raised past the caller.
